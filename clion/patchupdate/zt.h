...
inline FILE* cfile_impl(buffer_t* const fb){
    return (static_cast<io_buffer_t* const>(fb))->file(); //type std::__c_file
}

inline FILE* cfile(std::ofstream const& ofs){return cfile_impl(ofs.rdbuf());}
inline FILE* cfile(std::ifstream const& ifs){return cfile_impl(ifs.rdbuf());}



inline unsigned int zt_crc2(std::istream& is) {
    char buf[4096] = "";
    size_t n = 0;
    unsigned int crc = 0xFFFFFFFF;
    while((n = is.readsome(&buf[0], sizeof(buf))))
        crc = ~crc32(~crc, buf, n);
    return crc;
}

inline unsigned int zt2_crc32_zlib(unsigned char *message, size_t len, unsigned int crc = 0xFFFFFFFF) {
