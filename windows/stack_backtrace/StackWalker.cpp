...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%p %p (%s): %s: %s\n", (LPVOID) entry.offset, (LPVOID)(entry.offset - entry.baseOfImage), entry.moduleName, entry.lineFileName, entry.name);
    }
    else
